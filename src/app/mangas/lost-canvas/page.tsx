...
import lostCanvasVolumes from '../../data/lostCanvasVolumes'; // Caminho relativo para src/app/mangas/lost-canvas/page.tsx

export default function LostCanvas() {
  const allErasSections = lostCanvasVolumes.map(volume => `Volume ${volume.volume}`);

  const [selectedSection, setSelectedSection] = useState(allErasSections[0]); // Estado para a seção selecionada
  const selectedVolume = lostCanvasVolumes.find(
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 text-white ">
      <h1 className="text-3xl md:text-4xl font-extrabold text-yellow-500 text-center mb-8 md:mb-12 animate-fade-in-down">
        Os Cavaleiros do Zodíaco - Lost Canvas (Tankōbon)
      </h1>
